...
struct ScriptListView: View {
    ...
    var body: some View {
        NavigationStack {
                ...
                if filteredScripts.isEmpty {
                    Section {
                        VStack(alignment: .leading, spacing: 4) {
                            ...
                            Text(isPickerMode ? "导入文件或选择“无”。" : "点击“新建”或“导入”开始。")
                                .font(.caption)
                                .foregroundStyle(.secondary)
                        }
                        .padding(.vertical, 4)
            ...
            .alert("删除脚本？", isPresented: $showDeleteConfirmation, presenting: pendingDelete) { script in
                ...
                Text("删除 \(script.lastPathComponent)？此操作不可撤销。")
            }
            .fileImporter(
    ...
    private func saveDefaultScript(_ url: URL) {
        ...
        presentSuccess(title: "已设为默认", message: url.lastPathComponent)
    }
    ...
    private func createNewScript() {
        ...
        do {
            ...
            presentError(title: "创建文件失败", message: error.localizedDescription)
        }
    }
