...
private func tunnelConnectionAlertMessage(for error: NSError) -> String {
    ...
    if error.code == 48 || lowercasedMessage.contains("address already in use") || lowercasedMessage.contains("port already in use") {
        ...
        recoverySteps = [
            "关闭其它可能占用隧道的 JIT、调试、代理或 VPN 应用。",
            "断开并重新连接 LocalDevVPN。",
            "重启 StikDebug，然后重试。",
            "如果问题持续存在，请重启设备以释放被占用的端口。"
        ]
        ...
        recoverySteps = [
            ...
            "唤醒并解锁目标设备。",
            "确认 LocalDevVPN 正在 \(targetIP) 暴露设备。"
        ]
    ...
    目标：\(targetIP):49152
    预期 LocalDevVPN IP：\(DeviceConnectionContext.defaultTargetIPAddress)

    请尝试：
    \(steps)

    技术详情：
    错误代码 \(error.code)：\(rawMessage)
    """
}
