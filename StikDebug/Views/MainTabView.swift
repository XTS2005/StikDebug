            ...
            DispatchQueue.main.async {
                if code == 0 {
                    ...
                    LogManager.shared.addInfoLog(
                        String(format: "从 URL 模拟位置：%.6f, %.6f", coordinate.latitude, coordinate.longitude)
                    )
                } else {
                    showAlert(
                        title: "位置模拟失败",
                        message: "无法从 URL 模拟位置（错误 \(code)）。请确保设备已连接且 DDI 已挂载。",
                        showOk: true
                    )
                }
            }
        ...
        LocationSimulationCommandQueue.shared.async {
            ...
            DispatchQueue.main.async {
                if code == 0 {
                    ...
                    LogManager.shared.addInfoLog("从 URL 清除模拟位置")
                } else {
                    showAlert(
                        title: "清除位置失败",
                        message: "无法从 URL 清除模拟位置（错误 \(code)）。",
                        showOk: true
                    )
                }
            }
