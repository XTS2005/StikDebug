...
private enum CoordinateImportError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .emptyFile:
            return "所选文件为空。"
        case .noCoordinates:
            return "未找到有效坐标。请使用 GPX、GeoJSON、JSON、CSV 或包含经纬度的纯文本文件。"
        }
    }
}
...
struct LocationSimulationView: View {
            ...
            MapReader { proxy in
                Map(position: $position) {
                    if hasRouteContext {
                        ...
                        Marker("图钉", coordinate: coordinate)
                            .tint(.red)
                    }
                }
    ...
    private func simulateRoute() {
        ...
        runLocationCommand(
            errorTitle: "路线模拟失败",
            errorMessage: { code in
                "无法开始路线模拟（错误 \(code)）。请确保设备已连接且 DDI 已挂载。"
            },
            ...
            do {
                ...
                guard let route = response.routes.first else {
                    throw NSError(
                        ...
                        code: -1,
                        userInfo: [NSLocalizedDescriptionKey: "未返回可行驶的路线。"]
                    )
                }
            ...
            Group {
                if bookmarks.isEmpty {
                    ContentUnavailableView(
                        ...
                        description: Text("在地图上放置图钉，然后点击书签图标以保存位置。")
                    )
                } else {
                    List {
