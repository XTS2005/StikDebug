...
enum ProcessControlAction: String {
    ...
    func successMessage(for pid: Int) -> String {
        switch self {
        case .resume:
            return "已向 PID \(pid) 发送 SIGCONT（19）。"
        case .pause:
            return "已向 PID \(pid) 发送 SIGSTOP（17）。"
        case .kill:
            return "PID \(pid) 已终止。"
        }
    }
    ...
    func timeoutMessage(for pid: Int) -> String {
        switch self {
        case .resume:
            return "无法确认 PID \(pid) 的恢复。请重试。"
        case .pause:
            return "无法确认 PID \(pid) 的暂停。请重试。"
        case .kill:
            return "无法确认 PID \(pid) 的终止。请重试。"
        }
    }
}
    ...
    func control(_ action: ProcessControlAction, process: ProcessInfoEntry) {
        guard activeControlState == nil else {
            ...
            if let activeControlState {
                actionAlertMessage = "正在为 PID \(activeControlState.pid) \(activeControlState.action.progressTitle)。"
            } else {
                actionAlertMessage = "已有其它进程操作正在运行。"
            }
        ...
        Task.detached(priority: .userInitiated) { [weak self] in
            ...
            let errorMessage = err?.localizedDescription ?? "未知错误"
            await MainActor.run {
